 ...
 * @author bill lin
 */
/* remove by chenqiwang 统一在meta.jsp引入
 * $.get(base+"/sys/dictSpec/loadAllJson",function(value){
	dictMap=value;
});*/
layui.define(['laypage', 'layer', 'form','laydate'], function (exports) {
    ...
    var table = $('#dateTable').DataTable({
        ...
        ajax: {
            ...
            data: function (data) {
                data.dispatch_no = $.trim($("#dispatch_no_query").val());
                data.order_no_like = $.trim($("#order_no_query").val());
                data.carrier_code = $.trim($("#carrier_code_query").val());
                data.is_generate_cost = $.trim($("#is_generate_cost_query option:selected").val());
    ...
    function addFeeHanlder() {
    	...
    	if(selectRows.length === 1){
    		gotoForm("编辑费用", {id:selectRows[0].id, dispatch_no:selectRows[0].dispatch_no, order_no:selectRows[0].order_no});
    ...
    $("#main_drive_name_query").on('click', function () {
    	//url,title,width,hight,id,name,callback
    	top.xmtc.popUp(base+"/view/oms/cust/csTransportPersonnelWin.jsp","主驾",600,432,"personnel_number","personnel_name",function(res){
    ...
    function gotoForm(title, data) {
        var url = base+"/view/oms/dispatch/csDispatchOrderForm.jsp";
        if (data.id) {
            url += "?id=" + data.id+"&dispatch_no="+data.dispatch_no + "&order_no="+data.order_no;
        }
        layer_show(title, url, '100%','100%');
    }

    exports('csDispatchOrder', {});
});
