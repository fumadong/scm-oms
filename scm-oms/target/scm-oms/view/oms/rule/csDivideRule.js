...
layui.define(['laypage', 'layer', 'form'], function (exports) {
    ...
    var table= $('#dataTable').DataTable({
        ajax: {
            url: base + "/rule/csDivideRule/page",
            data: function(data){
    ...
    function deleteHanlder() {
        ...
        if (ids.length == 0) {
            ...
            layer.confirm('确认要删除吗，删除后数据不可恢复？', function (index) {
                xmtc.ajaxPost(base + "/rule/csDivideRule/delete", {id: ids.join(",")}, function (data) {
                    if (data.success) {
                        table.ajax.reload();
                        layer.msg('操作成功');
                    } else {
        ...
        if (ids.length == 0) {
            ...
            if (statusStr.join(",").indexOf("1") != -1) {
                ...
                layer.confirm('确认要启用吗？', function (index) {
                    xmtc.ajaxPost(base + "/rule/csDivideRule/updateStatus", {id: ids.join(","), status: "1"}, function (data) {
                        if (data.success) {
                            table.ajax.reload();
                            layer.msg('启用成功');
                        } else {
        ...
        if (ids.length == 0) {
            ...
            if (statusStr.join(",").indexOf("0") != -1) {
                ...
                layer.confirm('确认要停用吗？', function (index) {
                    xmtc.ajaxPost(base + "/rule/csDivideRule/updateStatus", {id: ids.join(","), status: "0"}, function (data) {
                        if (data.success) {
                            table.ajax.reload();
                            layer.msg('停用成功');
                        } else {
