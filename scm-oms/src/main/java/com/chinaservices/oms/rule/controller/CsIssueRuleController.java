...
*@author Noaman.Chen
*/
@Controller(controllerKey = "/rule/csIssueRule", viewPath = "/admin")
public class CsIssueRuleController extends BaseController{
    CsIssueRuleService csIssueRuleService = new CsIssueRuleService();
	/**
	 * 列表查询
	 */
    public void page() {
        ...
        Page<Record> list = new SqlExecutor().page("csIssueRule_qeury", params, pageNo, pageSize);
    ...
    public void delete() {
        ...
        if(ids.length>0){
            csIssueRuleService.deleteByIds(ids);
        }
    ...
    public void saveOrUpdate() {
        Result result = new Result();
        CsIssueRule model = getModel(CsIssueRule.class, "");
        Integer id = model.getInt("id");
        boolean isDuplicate = csIssueRuleService.checkDuplicateRuleCode(id, model.getStr("code"));
        if (isDuplicate) {
            result.setSuccess(false);
            result.addError("规则代码[" + model.getStr("code") + "]不能重复");
        } else {
            ...
            csIssueRuleService.saveOrUpdate(model);
            result.setSuccess(true);
    ...
    public void getById() {
        ...
        if (null != id) {
            CsIssueRule model = csIssueRuleService.findById(id);
            result.setData(model);
        } else {
            result.setData(null);
        }
    ...
    public void updateStatus() {
        ...
        String[] ids = StringUtils.split(id, ",");
        csIssueRuleService.updateStatus(ids, getLoginUserId(), status);
        result.setSuccess(true);
        renderJson(result);
    }
