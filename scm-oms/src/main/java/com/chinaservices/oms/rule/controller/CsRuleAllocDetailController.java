...
*@author Dylan Fu
*/
@Controller(controllerKey = "/rule/csRuleAllocDetail", viewPath = "/admin")
public class CsRuleAllocDetailController extends BaseController{
    ...
    public void query() {
        ...
        params.put("rule_alloc_header_id", getPara("rule_alloc_header_id"));
        
        int draw = getParaToInt("draw");
        int pageNo = (getParaToInt("start") / getParaToInt("length")) + 1;
        int pageSize = getParaToInt("length");
        Page<CsRuleAllocDetail> list = CsRuleAllocDetail.dao.getPageByCondition(params, pageNo, pageSize);
    ...
    public void delete() {
        ...
        for (String idStr : ids) {
            CsRuleAllocDetail.dao.deleteById(Integer.valueOf(idStr));
        }
    ...
    public void submit() {
        Result result = new Result();
        CsRuleAllocDetail csRuleAllocDetail = getModel(CsRuleAllocDetail.class, "");
        csRuleAllocDetail.put("tenancy",1);
        csRuleAllocDetail.put("creator",1);
        csRuleAllocDetail.put("create_time", DateUtil.now());
        csRuleAllocDetail.put("modifier",1);
        csRuleAllocDetail.put("modify_time",DateUtil.now());
        csRuleAllocDetail.put("rec_ver",1);
        csRuleAllocDetail.put("company_id",1);
        Integer id = csRuleAllocDetail.getInt("id");
        csRuleAllocHeader csCustomer = csRuleAllocHeader.dao.findById(csRuleAllocDetail.getInt("rule_alloc_header_id"));
        //设置仓库代码
        if(csCustomer != null){
        	csRuleAllocDetail.set("warehouse_code", csCustomer.get("warehouse_code"));
        	csRuleAllocDetail.set("rule_code", csCustomer.get("rule_code"));
        }
      //  boolean isDefault = CsRuleAllocDetail.dao.checkDefault(csRuleAllocDetail);
       // boolean isContactCode = CsRuleAllocDetail.dao.checkDuplicateNo(csRuleAllocDetail);
    /*    if(csRuleAllocDetail.getInt("is_default") !=null &&
        		csRuleAllocDetail.getInt("is_default")==1
        		&& isDefault){
        	 result.setSuccess(false);
             result.addError("1");
        }*/
       /*else {*/
            ...
            CsRuleAllocDetail.dao.saveOrUpdate(csRuleAllocDetail);
            // 生成对应的电子围栏并保存
    ...
    public void getById() {
        ...
        if (null != id) {
            CsRuleAllocDetail csRuleAllocDetail = CsRuleAllocDetail.dao.findById(id);
            result.setData(csRuleAllocDetail);
        } else {
            result.setData(null);
        }
        result.setSuccess(true);
